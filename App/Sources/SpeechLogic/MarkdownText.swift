...
enum MarkdownText {
    ...
    static func inlineRuns(_ line: String) -> [InlineRun] {
    ...
    /// handler.
    static func linkTargets(in markdown: String) -> [(label: String, url: String, range: Range<String.Index>)] {
    ...
    static func imageTokens(in markdown: String) -> [(alt: String, url: String, range: Range<String.Index>)] {
        guard let regex = try? NSRegularExpression(pattern: #"!\[[^\]]*\]\([^)]+\)"#) else {
            return []
        }
        let ns = markdown as NSString
