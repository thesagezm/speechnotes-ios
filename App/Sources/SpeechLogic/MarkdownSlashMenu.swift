...
enum MarkdownSlashMenu {
    ...
    struct Command: Identifiable, Hashable {
        ...
        internal init(id: String, label: String, symbol: String, snippet: String, placeholder: String? = nil) {
            self.id = id
            self.label = label
            self.symbol = symbol
    ...
    internal static let commands: [Command] = [
    ...
    struct Trigger {
        ...
        internal let slashIndex: String.Index
        /// Index immediately AFTER the `/` — what the user has typed.
        internal let cursorIndex: String.Index
    ...
    static func detect(in draft: String, caretOffset: Int) -> Trigger? {
    ...
    static func filter(prefix: String) -> [Command] {
    ...
    static func apply(_ command: Command, in draft: String, trigger: Trigger) -> (draft: String, caretUtf16: Int) {
        var updated = draft
