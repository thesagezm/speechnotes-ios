...
- [ ] Phase 2 acceptance: `kokoro-sample` artifact contains audible Kokoro speech; check RTF and the voice list in `kokoro-spike-log` — **demoted to non-blocking garnish** after 6 environment-only failures (all Xcode/SPM plumbing, Kokoro inference never ran); `continue-on-error: true` set. The iPhone is now the test bench.
