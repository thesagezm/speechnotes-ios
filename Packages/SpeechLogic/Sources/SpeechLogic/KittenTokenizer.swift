...
public enum KittenTokenizer {
    ...
    /// symbol → id, built exactly like the reference TextCleaner
    /// (`dicts[symbols[i]] = i`). The table is keyed and looked up per
    /// Unicode SCALAR, not per Character: Python iterates codepoints, and
    /// Swift grapheme clustering would merge "'" + U+0329 into one key,
    /// losing the combining-mark entry (and any base+mark input cluster).
    /// Last occurrence wins on duplicates: '"' occupies 11/14/15 → 15,
    /// "'" occupies 174/176 → 176.
    public static let vocabulary: [Unicode.Scalar: Int] = {
        var dict: [Unicode.Scalar: Int] = [:]
        for (index, scalar) in (pad + punctuation + letters + ipa).unicodeScalars.enumerated() {
            dict[scalar] = index
        }
        return dict
    }()
    ...
    /// Phonemes → token ids (no BOS/EOS added). Per-scalar lookup, so a
    /// base+combining-mark cluster keeps the base's id and drops the mark —
    /// exactly what the per-codepoint reference does.
    public static func ids(for phonemes: String) -> [Int] {
        regularizeSpacing(phonemes).unicodeScalars.compactMap { vocabulary[$0] }
    }
